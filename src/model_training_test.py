...
# Load the dataset
data = pd.read_csv(r'D:\Git Projects\Price_forecasting_project\Agricultural-Price-Intelligence\data\raw\processed\Azadpur\Misri_Fancy_dataset.csv')
...
train_data = available_data[available_data['ds'] < '2024-06-10']
test_data = available_data[available_data['ds'] >= '2024-06-10']
...
def random_forest_model(train_data, test_data):
    ...
    train_subset = train_data[train_data['ds'] < '2023-06-10']
    val_subset = train_data[(train_data['ds'] >= '2023-06-10') & (train_data['ds'] < '2024-06-10')]
...
def save_plot(y_true, y_pred, model_name, dates):
    ...
    plot_dir = os.path.join("model_results","Azadpur","Misri_Fancy", model_name)
    ...
    if args.model:
        if args.model in models_to_run:
            ...
            try:
                ...
                result_dir = os.path.join("model_results","Azadpur","Misri_Fancy", args.model)
        ...
        for model_name, model_func in models_to_run.items():
            try:
                ...
                # Save the results to a file
                result_dir = os.path.join("model_results","Azadpur", model_name)
                os.makedirs(result_dir, exist_ok=True)
